...
const EditProgram = ({ program, employees, onCancel, onSubmit, onDelete, onPreview }) => {
  ...
  // Fetch fresh program data when component mounts or program prop changes
  useEffect(() => {
    if (program && program.id) {
      const fetchFreshProgramData = async () => {
        setLoadingProgram(true)
        try {
          const token = getCookie(TOKEN)
          const response = await fetch(`${getApiBaseUrl()}/api/programs/${program.id}`, {
            headers: {
          ...
          if (response.ok) {
            ...
            const freshProgram = data.program
            
            // Transform to match UI format
            const transformedProgram = {
              ...freshProgram,
              id: freshProgram.program_id,
              owners: Array.isArray(freshProgram.owners) ? freshProgram.owners : [],
              participants: Array.isArray(freshProgram.participants) ? freshProgram.participants : []
            }
            
            setCurrentProgram(transformedProgram)
          } else {
            // If fetch fails, use the program from prop
  ...
  // Initialize form data when currentProgram changes
  useEffect(() => {
    if (currentProgram) {
      // Ensure modules have IDs when loading (preserve existing IDs, add new ones if missing)
      const modulesWithIds = ensureModuleIds(Array.isArray(currentProgram.modules) ? currentProgram.modules : [])
      
      setFormData({
        program_name: currentProgram.program_name || '',
        program_type: currentProgram.program_type || '',
        version: currentProgram.version || '',
        estimated_duration: currentProgram.estimated_duration || '',
        description: currentProgram.description || '',
        category_tags: currentProgram.category_tags || '',
        modules_data: currentProgram.modules_data || '',
        modules: modulesWithIds,
        owners: currentProgram.owners?.map(o => {
          // Handle both object and ID formats
          if (typeof o === 'object' && o !== null && o.id !== undefined) {
            return o.id
          }
          return typeof o === 'number' ? o : (o?.id || o)
        }) || [],
        resources: currentProgram.resources || [],
        participants: Array.isArray(currentProgram.participants) 
          ? currentProgram.participants.map(p => {
              // Handle both cases: array of IDs (numbers) or array of objects with id property
              if (typeof p === 'object' && p !== null && p.id !== undefined) {
                return p.id
              }
              // If it's already a number/ID, return as is
              return typeof p === 'number' ? p : (p?.id || p)
            })
          : []
      })
      
      // Clear any stored original edit values when program changes
      originalEditValuesRef.current = null
      setEditingModuleIndex(null)
      
      // If program type is Certification, try to find matching certificate
      if (currentProgram.program_type === 'Certification' && currentProgram.program_name) {
        const matchingCert = certificates.find(c => c.certificate_name === currentProgram.program_name)
        if (matchingCert) {
          setSelectedCertificate(matchingCert.certificate_id)
        }
      }
    }
  }, [currentProgram, certificates])
  ...
  // Show spinner while loading fresh program data
  if (loadingProgram) {
    return (
      <>
  ...
  return (
    <>
      ...
      <div className="animate-fade-in">
        ...
        <div className="card p-4 mb-2">
          {/* Form Header */}
          <div className="mb-2 pb-4 border-b border-slate-200 flex justify-between items-center">
            <div className="text-lg font-semibold text-slate-900">Edit Program</div>
                                  ...
                                  <React.Fragment key={moduleKey}>
                                    ...
                                    <tr className="bg-slate-50 hover:bg-slate-100 transition-colors">
                                      <td 
                                        ...
                                        rowSpan={(module.submodules && module.submodules.length) || 1}
                                      >
                                    ...
                                    {/* Submodule Rows */}
                                    {(module.submodules && module.submodules.length > 1) && module.submodules.slice(1).map((submodule, subIndex) => {
                    ...
                    <div className="p-6 overflow-y-auto max-h-[calc(90vh-180px)]">
                      ...
                      <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                        ...
                        <div className="mt-3 p-3 bg-green-100 rounded text-sm text-green-800">
                          <strong>Note:</strong> Modules and Submodules names are required. Start date, End date, and Url are optional but recommended. Enter module name in the first row, then leave Modules column empty for submodule rows.
                        </div>
                      </div>
                        ...
                        <div className="space-y-4">
                          ...
                          <div>
                            ...
                            <input
                              ...
                              className="hidden"
                              id="module-file-input-edit"
                            />
                            <label
                              htmlFor="module-file-input-edit"
                ...
                {expandedSections.resources && (
                  <div className="p-5 rounded-b-xl">
                    ...
                    <div className="text-xs text-slate-600">
                      Add URLs to course materials, videos, documents, GitHub repos, etc.
                    </div>
                  </div>
                )}
              </div>
              ...
              <div className="border border-slate-200 rounded-xl mb-4">
                ...
                >
                  <div className="flex items-center gap-2 font-semibold text-slate-900">
                    ...
                    Program Participants ({formData.participants.length})
                  </div>
