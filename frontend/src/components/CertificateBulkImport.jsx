...
const CertificateBulkImport = ({ onClose, onImportComplete }) => {
  ...
  const analyzeFile = async (file) => {
    ...
    try {
      ...
      const response = await fetch(`${getApiBaseUrl()}/api/certificates/bulk-import?analyze=true`, {
        method: 'POST',
        headers: {
  ...
  const handleImport = async (file) => {
    ...
    try {
      if (!hasPermission('matrix-import')) {
        setErrors(['You do not have permission to import certificates'])
        setLoading(false)
        return
      }
      
      if (!isAuthenticated()) {
        setErrors(['You must be logged in to import certificates'])
        setCurrentStep('upload')
        setLoading(false)
        return
      }
      ...
      const response = await fetch(`${getApiBaseUrl()}/api/certificates/bulk-import`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const downloadTemplate = () => {
    // Create a sample Excel template for Certificate Import
    const templateData = [
      ['Employee ID', 'First Name', 'Last Name', 'Certificate Name', 'Category', 'Status', 'Difficulty Level', 'Issued By', 'Start Date', 'End Date'],
      ['EMP001', 'John', 'Doe', 'AWS Certified Solutions Architect', 'Technical', 'Completed', 'Advanced', 'Amazon Web Services', new Date('2024-01-15'), new Date('2027-01-15')],
      ['EMP002', 'Jane', 'Smith', 'PMP Certification', 'Project Management', 'In-Progress', 'Intermediate', 'PMI', new Date('2024-03-01'), new Date('2027-03-01')],
      ['EMP003', 'Bob', 'Johnson', 'Google Cloud Professional', 'Technical', 'Completed', 'Advanced', 'Google', new Date('2024-02-10'), new Date('2027-02-10')]
    ]
    ...
    ws['!cols'] = [
      ...
      { wch: 12 }, // First Name
      { wch: 12 }, // Last Name
      { wch: 30 }, // Certificate Name
      { wch: 18 }, // Category
      { wch: 12 }, // Status
      { wch: 16 }, // Difficulty Level
      { wch: 20 }, // Issued By
      { wch: 15 }, // Start Date (wider for date display)
      { wch: 15 }  // End Date (wider for date display)
    ]

    // Format date columns (columns I, J are 0-indexed, so 8, 9)
    const dateColumns = [8, 9] // Start Date, End Date
    // Excel date format code - mm/dd/yyyy
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-slate-200">
          <h2 className="text-2xl font-semibold text-slate-900">Bulk Import Employee Certificates</h2>
          <button
            type="button"
            onClick={(e) => {
              e.preventDefault()
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-120px)]">
            ...
            <div>
              ...
              <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
                ...
                <div className="mt-3 p-3 bg-green-100 rounded text-sm text-green-800">
                  <strong>Note:</strong> Status should be one of: Completed, In-Progress. Date format should be YYYY-MM-DD.
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    <div className="flex items-center">
                      ...
                      <div>
                        <div className="text-2xl font-bold text-green-900">{analysisResult.new_certificates || 0}</div>
                        <div className="text-sm text-green-700">New Certificates</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                    <div className="flex items-center">
                      ...
                      <div>
                        <div className="text-2xl font-bold text-green-900">{analysisResult.assignments || 0}</div>
                        <div className="text-sm text-green-700">Certificate Assignments</div>
                      </div>
                    </div>
                  </div>
                    ...
                    <table className="w-full text-sm">
                      <thead className="bg-slate-50 sticky top-0">
                        <tr>
                          ...
                          <th className="px-3 py-2 text-left font-medium text-slate-700">Certificate</th>
                          <th className="px-3 py-2 text-left font-medium text-slate-700">Category</th>
                          <th className="px-3 py-2 text-left font-medium text-slate-700">Status</th>
                          <th className="px-3 py-2 text-left font-medium text-slate-700">Difficulty</th>
                          <th className="px-3 py-2 text-left font-medium text-slate-700">Start Date</th>
                      ...
                      <tbody className="divide-y divide-slate-200 bg-white">
                        {analysisResult.preview_data.map((row, index) => (
                          <tr key={index} className="hover:bg-slate-50">
                            ...
                            <td className="px-3 py-2">{row.certificate_name || 'N/A'}</td>
                            ...
                            <td className="px-3 py-2">{row.difficulty_level || '-'}</td>
          ...
          {currentStep === 'complete' && importResult && (
            <div className="text-center py-12">
              ...
              <div className="text-slate-600 mb-6 space-y-2">
                {importResult.certificates_created && (
                  <p><strong>{importResult.certificates_created}</strong> certificates added to management</p>
                )}
                {importResult.assignments_created && (
                  <p><strong>{importResult.assignments_created}</strong> certificate assignments created</p>
                )}
          ...
          {currentStep === 'importing' && (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-slate-900 mb-2">Importing Certificates...</h3>
...
export default CertificateBulkImport
